...
counter = 0
t2 = datetime.now()
print ("Starting photo to sequence")
for frame in camera.capture_continuous(capture, format="bgra", use_video_port=True, resize=(img_width,img_height)):
    ...
    if cntdwn_timer == -1:
      counter += 1
      filename = '/media/pi/IOT4/image/'+str(t2.strftime("%d-%m-%Y_%H-%M-%S"))+'_'+str(counter) + '.png'
    ...
    # Break the programe when receiving all the images
    if counter == total_photos:
      break
 
print ("Photo sequence finished")

src_dir = '/media/pi/IOT4/image'
...
for img in up_img:
    ...
    filename = '/media/pi/IOT4/img_upload/' + str(t2.strftime("%d-%m-%Y_%H-%M-%S"))+'.png'
